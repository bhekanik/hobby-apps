...
export const filterDeals = (deals, filter) => {
    ...
    const sectorsFiltered = countriesFiltered.filter((deal) => {
        ...
        (deal?.company.sector?.includes("[")
            ...
            : JSON.parse(`["${deal?.company.sector}"]`)).forEach((s) => {
    ...
    const otherFiltered = stagesFiltered.filter((deal) => {
        return (dateRange === null ||
            isWithinInterval(new Date(deal.press_release?.date), {
                start: typeof dateRange.start === "string"
                    ? new Date(dateRange.start)
                    : dateRange.start,
                end: typeof dateRange.end === "string"
...
//# sourceMappingURL=filterDeals.jsx.map