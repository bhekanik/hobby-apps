...
export const formatDealflow = (deals) => {
    ...
    const sectors = deals
        ...
        .flat()
        .filter(Boolean);
    const countryListMap = countryList.reduce((acc, country) => {
        acc[country.code] = country.name;
        return acc;
    }, {});
    const country = uniqueCountries.length > 1
        ? ["Africa"]
    ...
    return {
        filter: {
            ...
            dateRange: deals.length === 0
                ...
                : {
                    start: min([
                        ...new Set(deals.map((deal) => new Date(deal.press_release.date))),
                    ]),
                    end: max([
                        ...new Set(deals.map((deal) => new Date(deal.press_release.date))),
                    ]),
...
//# sourceMappingURL=formatDealflow.js.map